...
function userId(req: express.Request) { return getAuth(req).userId!; }
...
app.post("/api/sync", requireAuth(), async (req, res) => {
  ...
  await pool.query(
    `INSERT INTO users (id, email, name, id_card) VALUES ($1,$2,$3,$4)
  ...
  const { rows } = await pool.query(
    `SELECT u.id,u.email,u.name,u.status,u.is_admin,
            p.age,p.gender,p.department,p.year,p.photos,p.prompts,p.bio,p.interests,p.last_active
...
app.get("/api/me", requireAuth(), async (req, res) => {
  const { rows } = await pool.query(
    `SELECT u.id,u.email,u.name,u.status,u.is_admin,
            p.age,p.gender,p.department,p.year,p.photos,p.prompts,p.bio,p.interests,p.last_active
     FROM users u JOIN profiles p ON p.user_id=u.id WHERE u.id=$1`, [userId(req)]
  );
...
app.patch("/api/me", requireAuth(), async (req, res) => {
  ...
  await pool.query(
    `UPDATE profiles SET
       age=COALESCE($2::int,age), gender=COALESCE($3,gender),
       department=COALESCE($4,department), year=COALESCE($5::int,year),
...
app.get("/api/feed", requireAuth(), async (req, res) => {
  ...
  const { rows: me } = await pool.query(
    `SELECT u.status,p.gender FROM users u JOIN profiles p ON p.user_id=u.id WHERE u.id=$1`, [id]
  );
  ...
  const { rows } = await pool.query(
    `SELECT p.user_id,p.age,p.gender,p.department,p.year,p.photos,p.prompts,p.bio,p.interests,p.last_active,u.name
     FROM profiles p JOIN users u ON u.id=p.user_id
     WHERE u.id!=$1 AND u.status='approved' AND p.gender=$2
...
app.post("/api/likes", requireAuth(), async (req, res) => {
  ...
  const { rows: count } = await pool.query(
    `SELECT COUNT(*) FROM likes WHERE sender_id=$1 AND created_at>NOW()-INTERVAL '1 day'`, [sender]
  );
  if (Number(count[0].count) >= 8)
    return res.status(429).json({ error: "Daily like limit reached (8/day)." });
  ...
  if (mutual[0]) {
    const matchId = uid();
    await pool.query(`INSERT INTO matches (id,user1_id,user2_id) VALUES ($1,$2,$3)`, [matchId, sender, receiverId]);
    await pool.query(
...
app.get("/api/likes/received", requireAuth(), async (req, res) => {
  const { rows } = await pool.query(
    `SELECT l.id,l.item_id,l.item_type,l.message,l.created_at,
     ...
     ORDER BY l.created_at DESC`, [userId(req)]
  );
  res.json({ likes: rows });
});
...
app.post("/api/likes/:id/resolve", requireAuth(), async (req, res) => {
  ...
  const { rows } = await pool.query(
    `SELECT * FROM likes WHERE id=$1 AND receiver_id=$2`, [req.params.id, receiver]
  );
  if (!rows[0]) return res.status(404).json({ error: "Like not found." });
  if (action === "accept") {
    const matchId = uid();
    await pool.query(`INSERT INTO matches (id,user1_id,user2_id) VALUES ($1,$2,$3)`,
      [matchId, receiver, rows[0].sender_id]);
    if (rows[0].message)
      await pool.query(`INSERT INTO messages (id,match_id,sender_id,text) VALUES ($1,$2,$3,$4)`,
...
app.get("/api/matches/:id/messages", requireAuth(), async (req, res) => {
  const { rows } = await pool.query(
    `SELECT * FROM messages WHERE match_id=$1 ORDER BY created_at ASC`, [req.params.id]
  );
...
app.patch("/api/admin/users/:id", isAdmin, async (req, res) => {
  ...
  if (status === "banned")
    await pool.query(`DELETE FROM matches WHERE user1_id=$1 OR user2_id=$1`, [req.params.id]);
  res.json({ ok: true });
});
