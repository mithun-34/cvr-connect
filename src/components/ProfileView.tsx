...
export default function ProfileView({ profile, onProfileUpdate, onAdminClick }: Props) {
  ...
  function updatePrompt(idx: number, key: keyof PromptAnswer, val: string) {
    set("prompts", form.prompts.map((p, i) => i === idx ? { ...p, [key]: val } : p));
  }
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input className={inp} type="number" placeholder="Age" min={18} max={28}
          ...
          {form.prompts.map((p, i) => (
            <div key={p.id} className="bg-zinc-50 rounded-2xl p-3 border border-zinc-100 space-y-2">
              <div className="flex items-start gap-2">
                <select className={inp + " flex-1 text-xs"} value={p.question}
                ...
                <button onClick={() => removePrompt(i)}
                  className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-zinc-200 transition-colors shrink-0 mt-1">
                  <X className="w-3.5 h-3.5 text-zinc-500" />
                </button>
              </div>
              <textarea className={inp + " resize-none text-xs"} rows={2} maxLength={200}
                placeholder="Your answer…" value={p.answer}
